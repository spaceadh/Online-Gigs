...
def local_search(individual):
    # Choose a random local search operation
    operation = np.random.choice(['swap', 'insert', 'invert', 'shuffle', 'reverse'])
    print("Local Search operation chosen: ", operation)
    ...
    if operation == 'swap':
        ...
        idx1, idx2 = np.random.choice(len(individual), 2, replace=False)
        job_to_insert = individual.pop(idx1)
        individual.insert(idx2, job_to_insert)
        ...
        idx1, idx2 = np.random.choice(len(individual), 2, replace=False)
        subsequence = individual[idx1:idx2+1]
        np.random.shuffle(subsequence)
        individual[idx1:idx2+1] = subsequence
